...
int main ()
{
	...
	if (!sptime)
	{
		eqtime = SimDMP (idata, dmpdata, MAXTIME);
		SimGillespie (idata, gilldata, eqtime);
	}
	else
	{
		SimDMP (idata, dmpdata, MAXTIME);
		SimGillespie (idata, gilldata, MAXTIME);
	}

	//insert OUTROW of each data into outdata, then sort outdata by truetime
	for (int a = 0;a < OUTROW;a++)
	{
		static int b;
		b = (double)a * dmpdata.val.size () / OUTROW;
		if (b == (int)((double)(a - 1) * (int)dmpdata.val.size () / OUTROW))
			continue;
		outdata.push_back (OutputData ());
		outdata.back ().truetime = dmpdata.truetime[b];
		outdata.back ().val[0] = dmpdata.val[b];
	}
	...
	sort (outdata.begin (), outdata.end (), OutDataComp);

	//fill out blank spots with slope estimation
	outuntil = outdata.size ();
	lastval[0] = lastval[1] = make_pair (0, idata.init);
	for (int a = 0, b;a < outdata.size ();a++)
	{
		if (outdata[a].val[0].empty ())
		{
			for (b = a; b < outdata.size (); b++)
				if (!outdata[b].val[0].empty ())
					break;
			if (b == outdata.size ())
			{
				nextval.first = outdata.back ().truetime;
				nextval.second = outdata.back ().val[1];
				outuntil = min (outuntil, a);
			}
			else
			{
				nextval.first = outdata[b].truetime;
				nextval.second = outdata[b].val[0];
			}
			
			for (;a < b;a++)
			{
				outdata[a].val[0].resize (6);
				for (int c = 0;c < 6;c++)
					outdata[a].val[0][c] = lastval[0].second[c] + (outdata[a].truetime - lastval[0].first) * (nextval.second[c] - lastval[0].second[c]) / (nextval.first - lastval[0].first);
			}
		...
		{
			lastval[0].first = outdata[a].truetime;
			lastval[0].second = outdata[a].val[0];
		}
	}
	for (int a = 0, b; a < outdata.size (); a++)
	{
		if (outdata[a].val[1].empty ())
		{
			for (b = a; b < outdata.size (); b++)
				if (!outdata[b].val[1].empty ())
					break;
			if (b == outdata.size ())
			{
				nextval.first = outdata.back ().truetime;
				nextval.second = outdata.back ().val[0];
				outuntil = min (outuntil, a);
			}
			else
			{
				nextval.first = outdata[b].truetime;
				nextval.second = outdata[b].val[1];
			}

			for (; a < b; a++)
			{
				outdata[a].val[1].resize (6);
				for (int c = 0; c < 6; c++)
					outdata[a].val[1][c] = lastval[1].second[c] + (outdata[a].truetime - lastval[1].first) * (nextval.second[c] - lastval[1].second[c]) / (nextval.first - lastval[1].first);
			}
		...
		{
			lastval[1].first = outdata[a].truetime;
			lastval[1].second = outdata[a].val[1];
		}
	}

	//output rdata
	out << fixed << setprecision (10);
	out << "truetime," << "DMP:B," << "Gillespie:B\n";
	for (int b = 0; b < outuntil; b++)
		out << outdata[b].truetime << "," << outdata[b].val[0][2] << "," << outdata[b].val[1][2] << "\n";
	/*for (int b = 0;b < outdata.size ();b++)
	{
		out << outdata[b].truetime;
		if (outdata[b].val[0].empty ())
			out << ",," << outdata[b].val[1][2] << "\n";
		else
			out << "," << outdata[b].val[0][2] << ",\n";
	}*/
	out.close ();
