...
double SimDMP (InitData &idata, ResultData &rdata, double endtime)
{
	...
	for (int a = 1; rdata.truetime.back () < endtime; a++)
	{
		...
		{
			if (rdata.truetime.back () >= idata.pulse[c].time[0] && rdata.truetime.back () < idata.pulse[c].time[1])
				//make sure pulse doesn't put var under 0
				rdata.val[a][idata.pulse[c].var] = max ((double)0, rdata.val[a][idata.pulse[c].var] + idata.pulse[c].add * stepsize);
		}
...
int main ()
{
	...
	for (int a = 1; a < OUTROW; a++)
	{
		...
		outdata.back ().val[1] = dmpdata.val[b];
	}
