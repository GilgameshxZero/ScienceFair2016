...
namespace SFVG
{
	...
	LRESULT Slider::OnPaint (WPARAM wparam, LPARAM lparam)
	{
		...
		RECT box;
		box.top = 0;
		box.left = static_cast<LONG>((double)(1.0 + pos) / 2 * (client.right - client.bottom));
	...
	LRESULT Slider::OnLButtonDown (WPARAM wparam, LPARAM lparam)
	{
		...
		int xPos = GET_X_LPARAM (lparam),
			...
			boxleft = static_cast<int>((1 + pos) / 2 * (client.right - client.bottom)),
